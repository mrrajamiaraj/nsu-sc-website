...
export default async function NewAlumniPage({ searchParams }: { searchParams: { classYear?: string } }) {
  ...
  const presetYear = classYears.find((year) => year.id === searchParams.classYear);
    ...
    <div className="mx-auto max-w-2xl">
      ...
      {classYears.length === 0 ? (
        ...
        <GlassCard className="mt-6">
          <AlumniForm classYears={classYears} defaultClassYearId={presetYear?.id} action={createAlumni} />
        </GlassCard>
      )}
    </div>
  );
